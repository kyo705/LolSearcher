...
현재 프로젝트에서 REST API 통신 속도가 굉장히 느리다. 어떻게하면 REST API 속도를 높일 수 있을까 고민하던 중 현재의 REST API 데이터를 받아올 떄, blocking 형태로 데이터를 받아올 떄까지 기다리는 구조였다. 그래서 해당 방식을 non-blocking 방식으로 처리하면 많은 양의 데이터를 REST API로 받아올 때 속도를 향상시킬 수 있을 것이라 판단하였고 현재 구현하고 있는 상태다. 
