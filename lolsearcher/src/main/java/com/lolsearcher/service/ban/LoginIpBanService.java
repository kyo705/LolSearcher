...
import static com.lolsearcher.constant.RedisCacheConstants.LOGIN_ABUSING_KEY;
...
public class LoginIpBanService implements IpBanService {
	...
	public boolean isExceedBanCount(String ipAddress) {
		...
		return ((Integer) loginAbusingCache.get(ipAddress).get()) >= LOGIN_BAN_COUNT;
	}
	
	@Override
	public void registerBanList(String ipAddress) {
		requireNonNull(rediscCacheManager.getCache(LOGIN_BAN_KEY)).put(ipAddress, System.currentTimeMillis());

		requireNonNull(rediscCacheManager.getCache(LOGIN_ABUSING_KEY)).evictIfPresent(ipAddress);
	}
}
