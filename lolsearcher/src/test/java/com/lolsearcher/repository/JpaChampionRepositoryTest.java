...
class JpaChampionRepositoryTest {
	...
	public void findChampionStatic() {
		...
		assertThat(middleChampions.get(0).getCk().getChampionId()).isEqualTo("카타리나");
		
		//wins+losses 로 정렬해서 받기 때문에 엘리스,탈론 순으로 list채워짐(엘리스 게임수(140판), 탈론 게임수(90판))
		assertThat(jungleChampions.size()).isEqualTo(2);
		assertThat(jungleChampions.get(0).getCk().getChampionId()).isEqualTo("엘리스"); 
		assertThat(jungleChampions.get(1).getCk().getChampionId()).isEqualTo("탈론");
	}
}
