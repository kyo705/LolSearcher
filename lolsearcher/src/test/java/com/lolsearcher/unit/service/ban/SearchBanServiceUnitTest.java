...
public class SearchBanServiceUnitTest {
    ...
    @BeforeEach
    void setup(){
        given(cacheManager.getCache(SEARCH_BAN)).willReturn(cache);

        ipBanService = new SearchIpBanService(cacheManager);
    }
    ...
    public void addBanCount(){
        ...
        //given
        String ipAddress = "ip";
        given(cache.get(ipAddress, Integer.class)).willReturn(0);
    ...
    @ParameterizedTest
    @ValueSource(ints = {SEARCH_BAN_COUNT, SEARCH_BAN_COUNT+1, Integer.MAX_VALUE})
    ...
    @ParameterizedTest
    @ValueSource(ints = {SEARCH_BAN_COUNT-1, 0})
